...
function neighborBasedConstructiveCrossover(initialParent1, initialParent2, costMatrix){
...
/* Window-based Constructive Crossover
The algorithm for the WCX is as follows:
Step 1: - Start from 'node 1’ (i.e., current node p =1).
Step 2: - Search both of the parent chromosomes and consider the two-two neighboring ‘legitimate nodes' (the nodes that are not yet visited) of  'node p’ in each parent. If no 'legitimate node' after 'node p’ is present in any of the parent, search sequentially the nodes {2, 3, …, n} and consider the first 'legitimate' node, and go to Step 3.
Step 3: Suppose the 'node α', ‘node β', ‘node γ‘, ‘node δ’ and 'node ε ', ‘node κ', ‘node λ‘, ‘node ω’ are found in 1st and 2nd parent respectively, then for selecting the next node go to Step 4.
Step 4: Compare of the costs of adding all nodes to the ‘node p’ and then select the node with the minimum cost and concatenate it to the partially constructed offspring chromosome. If the offspring is a complete chromosome, then stop, otherwise, rename the present node as 'node p' and go to Step 2.
*/
function windowBasedConstructiveCrossover(initialParent1, initialParent2, costMatrix){
		parent1 = initialParent1.pop; //get the path stored in initialParent1
		parent2 = initialParent2.pop; // get the path stored in initialParent2 
		var dup_p1 = parent1.slice(); //duplicate copy of parent1
		var dup_p2 = parent2.slice(); //duplicate copy of parent2
		...
		var total_cost = 0;
		var considered = [dup_p1[0]];
		var current_position_p1 = 2;
		var current_position_p2 = 2;
		var element_window = [];
		var len = dup_p1.length;
		...
		while(counter <  len)
		{
			//create window of neighboring elements
			//first array
			element_window.push(parent1[current_position_p1-2]);
			element_window.push(parent1[current_position_p1-1]);
			element_window.push(parent1[current_position_p1+1]);
			element_window.push(parent1[current_position_p1+2]);
	
			//second array
			element_window.push(parent2[current_position_p2-2]);
			element_window.push(parent2[current_position_p2-1]);
			element_window.push(parent2[current_position_p2+1]);
			element_window.push(parent2[current_position_p2+2]);
			element_window = element_window.filter( onlyUnique );
...
module.exports = {
	...
	if(cx == 'ncx'){
			...
			//Do crossover and add the new offspring to the newPop
                	newPop.push(neighborBasedConstructiveCrossover(popFit[index1],popFit[index2],cMat));
                	counter++;
       		}
	}else if(cx == 'scx'){
		while(counter < crossoverNo){
		...
		while(counter < crossoverNo){
			...
			//Do crossover and add the new offspring to the newPop
                	newPop.push(windowBasedConstructiveCrossover(popFit[index1],popFit[index2],cMat));
                	counter++;
       		}
