...
while(counter <= 6 )
{
	...
	element_window.push(a[current_position-1]);
	element_window.push(a[current_position+1]);

	element_window.push(b[current_position_b-1]);
	element_window.push(b[current_position_b+1]);
	...
	else	
	{
		for(var k = 1; k < 8; k++)
		{
			...
			{	index = k;
				temp_cost = matrix[current_element-1][k-1];	
	...
	current_position = dup_a.indexOf(index)+1;
